import AxisRenderer from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/renderer/AxisRenderer";
...
import type { XAxis } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/XAxis";
...
import FSize from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/utils/FSize";
...
import { LimitLabelPosition } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/LimitLine";
import type LimitLine from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/LimitLine";
...
import { XAxisLabelPosition } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/XAxisLabelPosition";
...
export default class XAxisRenderer extends AxisRenderer {
    ...
    public renderAxisLabels(c: CanvasRenderingContext2D, isHorizontalFlip: boolean): void {
        try {
            LogUtil.log("XAxisRenderer renderAxisLabels - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.renderXAxisLabels);
            ...
            LogUtil.error("XAxisRenderer renderAxisLabels error", e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.renderXAxisLabels);
        }
        finally {
            LogUtil.log("XAxisRenderer renderAxisLabels - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.renderXAxisLabels);
        }
    }
    public renderAxisLine(c: CanvasRenderingContext2D): void {
        try {
            LogUtil.log("XAxisRenderer renderAxisLine - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.renderXAxisLine);
            ...
            LogUtil.error("XAxisRenderer renderAxisLine error", e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.renderXAxisLine);
        }
        finally {
            LogUtil.log("XAxisRenderer renderAxisLine - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.renderXAxisLine);
        }
    }
    /**
    ...
    protected drawLabels(c: CanvasRenderingContext2D, pos: number, anchor: MPPointF, isHorizontalFlip: boolean): void {
        ...
        for (let i = 0; i < positions.length; i += 2) {
            ...
            if (this.mViewPortHandler && this.mViewPortHandler.isInBoundsX(x)) {
                ...
                const width = Utils.getLabelTextSize(this.mAxisLabelPaint, label, this.textSizeCache).width;
                const height = this.mXAxis.mLabelRotatedHeight;
                labelInfo = {
                    value: label,
                    x: x - width / 2,
                    y: pos,
        ...
        this.mXAxis.setLabelXInfos(labelInfos);
    }
    ...
    protected drawXAxisValue(c: CanvasRenderingContext2D, text: string, x: number, y: number, paint: Paint, anchor: MPPointF, angleDegrees: number, labelPosition: XAxisLabelPosition = XAxisLabelPosition.Center, isHorizontalFlip: boolean = false): void {
        ...
        if (angleDegrees != 0.0) {
            ...
            if (anchor.x != 0.5 || anchor.y != 0.5) {
                const rotatedSize: FSize = Utils.getSizeOfRotatedRectangleByDegrees(textSize.width, lineHeight, angleDegrees);
    ...
    public renderGridLines(c: CanvasRenderingContext2D): void {
        try {
            LogUtil.log("XAxisRenderer renderGridLines - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.renderGridLines);
            ...
            LogUtil.error("XAxisRenderer renderGridLines error", e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.renderGridLines);
        }
        finally {
            LogUtil.log("XAxisRenderer renderGridLines - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.renderGridLines);
        }
    }
    ...
    private renderLinesInner(c: CanvasRenderingContext2D, lines: JArrayList<LimitLine>) {
        try {
            LogUtil.log("renderLinesInner - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.renderLinesInner);
            ...
            LogUtil.error("renderLinesInner error", e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.renderLinesInner);
        }
        finally {
            LogUtil.log("renderLinesInner - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.renderLinesInner);
        }
    }
    /**
    ...
    public renderLimitLineLine(c: CanvasRenderingContext2D, limitLine: LimitLine, position: number[]): void {
        try {
            LogUtil.log("XAxisRenderer renderXAxisLimitLine - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.renderXAxisLimitLine);
            ...
            LogUtil.error("XAxisRenderer renderXAxisLimitLine error", e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.renderXAxisLimitLine);
        }
        finally {
            LogUtil.log("XAxisRenderer renderXAxisLimitLine - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.renderXAxisLimitLine);
        }
    }
    public renderLimitLineLabel(c: CanvasRenderingContext2D, limitLine: LimitLine, position: number[], yOffset: number): void {
        try {
            LogUtil.log("renderLimitLineLabel - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.renderLimitLineLabel);
            ...
            LogUtil.info("renderLimitLineLabel succeed  " + `label:${JSON.stringify(label)}`);
        }
        catch (e) {
            LogUtil.error("renderLimitLineLabel error", e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.renderLimitLineLabel);
        }
        finally {
            LogUtil.log("renderLimitLineLabel - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.renderLimitLineLabel);
        }
    }
}
