import { MPPointF } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/index";
import type { ChartInterface, ChartPixelMap, Highlight, IValueFormatter } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/index";
import type ChartAnimator from '../animation/ChartAnimator';
import type BarBuffer from '../buffer/BarBuffer';
import HorizontalBarBuffer from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/buffer/HorizontalBarBuffer";
...
export default class HorizontalBarChartRenderer extends BarChartRenderer {
    ...
    drawDataSet(c: CanvasRenderingContext2D, dataSet: IBarDataSet, index: number) {
        try {
            LogUtil.log('HorizontalBarChartRenderer drawDataSet - start');
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.HorizontalBarChartDrawData);
            ...
            LogUtil.info('HorizontalBarChartRenderer drawDataSet succeed ' + `X:${JSON.stringify(phaseX)} Y:${JSON.stringify(phaseY)} ` + `trans:${JSON.stringify(trans)}`);
        }
        catch (e) {
            MPChartTraceUtil.startError(TraceLogConstants.Tag.HorizontalBarChartDrawData);
            LogUtil.error('HorizontalBarChartRenderer drawDataSet error', e);
        }
        finally {
            LogUtil.log('HorizontalBarChartRenderer drawDataSet - end');
            MPChartTraceUtil.finish(TraceLogConstants.Tag.HorizontalBarChartDrawData);
        }
    }
    drawValues(c: CanvasRenderingContext2D) {
        ...
        if (this.isDrawingValuesAllowed(this.mChart)) {
            ...
            const valueOffsetPlus: number = Utils.handleDataValues(5);
            let posOffset: number = 0;
            let negOffset: number = 0;
            const drawValueAboveBar: boolean = this.mChart.isDrawValueAboveBarEnabled();
            let halfTextHeight: number = Utils.getLabelTextSize(this.mValuePaint, "10", this.textSizeCache).height / 2;
            for (let i = 0; i < chartData.getDataSetCount(); i++) {
                ...
                if (!dataSet.isStacked()) {
                    for (let j = 0; j < buffer.buffer.length * this.mAnimator.getPhaseX(); j += 4) {
                        ...
                        let valueTextWidth: number = Utils.getLabelTextSize(this.mValuePaint, formattedValue, this.textSizeCache)
                            .width;
                        ...
                        if (entry.getIcon() != null && dataSet.isDrawIconsEnabled()) {
                            ...
                            py += iconsOffset.y;
                            if (icon) {
                                Utils.drawImage(c, icon, Math.floor(px), Math.floor(py), icon.getWidth(), icon.getHeight());
                            }
                        }
                    }
    ...
    drawHorizontalValue(c: CanvasRenderingContext2D, valueText: string, x: number, y: number, color: number) {
        ...
        Utils.resetContext2DWithoutLine(c, this.mValuePaint);
        c.beginPath();
        c.fillText(valueText, x, y);
        c.closePath();
    }
