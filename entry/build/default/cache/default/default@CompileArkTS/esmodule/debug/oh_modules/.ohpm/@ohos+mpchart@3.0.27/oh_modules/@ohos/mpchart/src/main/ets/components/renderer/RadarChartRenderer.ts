...
import type RadarEntry from '../data/RadarEntry';
...
export default class RadarChartRenderer extends LineRadarRenderer {
    ...
    public drawData(c: CanvasRenderingContext2D) {
        try {
            LogUtil.log("RadarChartRenderer drawData - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.RadarChartSetData);
            ...
            LogUtil.info("RadarChartRenderer drawData succeed  " + `radarData:${JSON.stringify(radarData)}`);
        }
        catch (e) {
            LogUtil.error("RadarChartRenderer drawData error", e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.RadarChartSetData);
        }
        finally {
            LogUtil.log("RadarChartRenderer drawData - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.RadarChartSetData);
        }
    }
    ...
    public drawValues(c: CanvasRenderingContext2D) {
        ...
        const yoffset = Utils.handleDataValues(5);
        for (let i = 0; i < this.mChart!.getData()!.getDataSetCount(); i++) {
            ...
            for (let j = 0; j < dataSet.getEntryCount(); j++) {
                ...
                if (entry.getIcon() != null && dataSet.isDrawIconsEnabled()) {
                    ...
                    if (icon) {
                        Utils.drawImage(c, icon, Math.floor(pIcon.x), Math.floor(pIcon.y), icon.getWidth(), icon.getHeight());
                    }
                }
            }
        ...
        if (maxCountEntrySet) {
            ...
            for (let i = 0; i < maxEntryCount; i += xIncrements) {
                if (center) {
                    ...
                    Utils.resetContext2DWithoutFont(c, this.mWebPaint);
                    c.beginPath();
    ...
    public drawHighlightCircle(c: CanvasRenderingContext2D, point: MPPointF, innerRadius: number, outerRadius: number, fillColor: number, strokeColor: number, strokeWidth: number): void {
        c.save();
        outerRadius = Utils.handleDataValues(outerRadius);
        innerRadius = Utils.handleDataValues(innerRadius);
        if (fillColor !== ColorTemplate.COLOR_NONE) {
            ...
            Utils.resetContext2DWithoutFont(c, this.mHighlightCirclePaint);
        ...
        if (strokeColor !== ColorTemplate.COLOR_NONE) {
            ...
            this.mHighlightCirclePaint.setStrokeWidth(Utils.handleDataValues(strokeWidth));
            Utils.resetContext2DWithoutFont(c, this.mHighlightCirclePaint);
            c.beginPath();
