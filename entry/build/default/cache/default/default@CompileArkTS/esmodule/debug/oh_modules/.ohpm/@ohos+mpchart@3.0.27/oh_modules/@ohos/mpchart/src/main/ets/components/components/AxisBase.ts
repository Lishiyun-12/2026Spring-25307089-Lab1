...
import DefaultAxisValueFormatter from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/formatter/DefaultAxisValueFormatter";
...
import LimitLine from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/LimitLine";
import type { GridLineConfig } from '../interfaces/LineConfig/CustomGridLineConfig';
...
export default abstract class AxisBase extends ComponentBase {
    ...
    textSizeCache: HashMap<string, TextMetrics> = new HashMap();
    /**
    ...
    /**
     * 设置GridLine的不透明度
     * @param alpha 不透明度，取值0到255之间，0是完全透明，255是完全不透明
     */
    ...
    /**
     * Sets the width of the border surrounding the chart in vp.
     *
     * @param width
     */
    public setAxisLineWidth(width: number): void {
        this.mAxisLineWidth = Utils.handleDataValues(width);
    }
    /**
    ...
    public setGridLineWidth(width: number): void {
        this.mGridLineWidth = Utils.handleDataValues(width);
    }
    /**
    ...
    public addLimitLine(l: LimitLine): void {
        ...
        if (this.mLimitLines.size() > 6) {
            LogUtil.log("MPChart", "Warning! You have more than 6 LimitLines on your axis, do you really want " +
                "that?");
        }
    }
