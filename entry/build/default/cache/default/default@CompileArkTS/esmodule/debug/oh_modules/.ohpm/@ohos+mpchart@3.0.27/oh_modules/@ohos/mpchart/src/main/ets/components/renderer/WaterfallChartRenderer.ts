...
import WaterfallBuffer from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/buffer/WaterfallBuffer";
import { WaterfallHighlight } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/data/WaterfallEntry";
import type WaterfallEntry from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/data/WaterfallEntry";
import type WaterfallDataSet from '../data/WaterfallDataSet';
...
export default class WaterfallChartRenderer extends BarChartRenderer {
    ...
    public initBuffers(): void {
        ...
        if (data) {
            ...
            for (let i = 0; i < this.mBarBuffers.length; i++) {
                ...
                if (dataSet) {
                    this.mBarBuffers[i] = new WaterfallBuffer(dataSet.getEntryCount() * 4 * (dataSet.isStacked() ? dataSet.getStackSize() : 1), data.getDataSetCount(), dataSet.isStacked());
                }
            }
        }
    }
    ...
    public drawData(c: CanvasRenderingContext2D): void {
        LogUtil.log('WaterfallChartRenderer drawData - start');
        MPChartTraceUtil.startInfo(TraceLogConstants.Tag.WaterfallChartSetData);
        try {
            if (!this.mChart) {
                return;
            }
            ...
            LogUtil.info('WaterfallChartRenderer drawData succeed  ' + `data:${JSON.stringify(data)}`);
        }
        catch (e) {
            LogUtil.error('WaterfallChartRenderer drawData error', e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.WaterfallChartSetData);
        }
        finally {
            LogUtil.log('WaterfallChartRenderer drawData - end');
            MPChartTraceUtil.finish(TraceLogConstants.Tag.WaterfallChartSetData);
        }
    }
    ...
    public drawHighlighted(c: CanvasRenderingContext2D, indices: Highlight[]): void {
        let isAccess: boolean = AppStorage.get(Constants.WATER_CHART_ACCESS) as boolean;
        if (!this.mChart) {
            return;
        }
        let waterfallData: WaterfallData | null = this.mChart.getWaterfallData();
