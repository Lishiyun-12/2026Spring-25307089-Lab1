...
export default abstract class Utils {
    ...
    public static calcText(paint: Paint, demoText: string): TextMetrics {
        ...
        if (demoText === undefined || demoText === null) {
            demoText = '';
        }
    ...
    public static async drawImage(canvas: CanvasRenderingContext2D, drawable: ChartPixelMap, x: number, y: number, width: number, height: number): Promise<void> {
    ...
    public static drawXAxisValue(c: CanvasRenderingContext2D, text: string, x: number, y: number, paint: Paint, anchor: MPPointF, angleDegrees: number, labelPosition: XAxisLabelPosition = XAxisLabelPosition.Center, isHorizontalFlip: boolean = false): void {
    ...
    public static getActualLength(value: string | number | undefined, length: number): number {
        if (!value) {
            return 0;
        }
    ...
    public static getAccessibilityHoverText(labelList: LabelInfo[], x: number, y: number) {
        let labelInfo: LabelInfo = { value: '' };
        if (labelList.length >= 0) {
            labelList.forEach((item) => {
                ...
                if (x >= item.x! && x <= item.x! + item.width! && y >= item.y! && y <= item.y! + item.height!) {
                    labelInfo.value = `${item.type}:${item.value}`;
                    labelInfo.x = item.x;
                    labelInfo.y = item.y;
                    labelInfo.width = item.width;
                    labelInfo.height = item.height;
                }
            });
        }
        return labelInfo;
    }
    public static matchingTemplate(formatter: string, replacements: Record<string, string>): string {
        // 构建正则表达式，匹配所有 {key} 格式的占位符
        const pattern = new RegExp(Object.keys(replacements)
            .map(key => key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')) // 转义特殊字符
            .join('|'), 'g');
        const replacer = (match: string): string => {
            return replacements[match] || match;
        };
        return formatter.replace(pattern, replacer);
    }
}
/**
 * 矩形圆角绘制位置类型，顶部/底部/都有
