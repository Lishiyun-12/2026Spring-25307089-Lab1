...
import ChartTouchListener, { ChartGesture } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/listener/ChartTouchListener";
import type BarLineChartBase from '../charts/BarLineChartBaseModel';
...
import type OnChartGestureListener from './OnChartGestureListener';
...
export default class BarLineChartTouchListener extends ChartTouchListener<BarLineChartBase<BarLineScatterCandleBubbleData<IBarLineScatterCandleBubbleDataSet<EntryOhos>>>> {
    ...
    actionMove(isTouchEvent: boolean, event: TouchEvent | GestureEvent) {
        ...
        if (this.mTouchMode == ChartTouchListener.DRAG) {
            ...
            && Math.abs(ChartTouchListener.distance(xCoordinate, this.mTouchStartPoint.x, yCoordinate, this.mTouchStartPoint.y)) > this.mDragTriggerDist && this.mChart?.getViewPortHandler().isInBounds(xCoordinate, yCoordinate)) {
    ...
    private calculateLongPress(event: TouchEvent): void {
        ...
        if (this.downTime > 0 && this.downPointX !== -1 && (curTime - this.downTime) >= this.mChart.getLongPressDuration()) {
            ...
            if (this.maxSwipePointX <= 1 && this.maxSwipePointY <= 1) {
    ...
    public onAccessUp(e: AccessibilityHoverEvent): void {
        if (!e) {
            return;
        }
        this.accessDown(e);
    ...
    accessDown(event: AccessibilityHoverEvent): void {
        if (!this.mChart.isDragEnabled() && (!this.mChart.isScaleXEnabled() && !this.mChart.isScaleYEnabled())) {
            return;
        }
        event = event as AccessibilityHoverEvent;
        let xCoordinate: number = event.x;
        let yCoordinate: number = event.y;
        this.mStartPointX = xCoordinate;
        this.mStartPointY = yCoordinate;
        this.stopDeceleration();
        this.saveAccessStart(event);
    }
    accessUp(event: AccessibilityHoverEvent): void {
        if (!event) {
            return;
        }
        let xCoordinate: number = 0;
    ...
    private saveAccessStart(event: AccessibilityHoverEvent) {
        this.mSavedMatrix.set(this.mMatrix);
        let e = event as AccessibilityHoverEvent;
        this.mTouchStartPoint.x = e.x;
        this.mTouchStartPoint.y = e.y;
        this.mClosestDataSetToTouch = this.mChart.getDataSetByTouchPoint(this.mTouchStartPoint.x, this.mTouchStartPoint.y);
    }
    ...
    public computeScroll(event: GestureEvent) {
        ...
        if (!!this.animatorResult) {
            ...
            if (uiContext) {
                this.animatorResult = uiContext.createAnimator(options);
            }
            else {
                return;
            }
