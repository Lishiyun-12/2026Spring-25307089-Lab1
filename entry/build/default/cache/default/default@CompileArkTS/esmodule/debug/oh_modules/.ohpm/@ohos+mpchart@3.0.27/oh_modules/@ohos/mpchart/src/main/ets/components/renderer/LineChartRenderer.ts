...
import { LineRadarRenderer } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/renderer/LineRadarRenderer";
...
export default class LineChartRenderer extends LineRadarRenderer {
    ...
    public mCirclePaintInner: Paint = new Paint();
    ...
    public drawData(c: CanvasRenderingContext2D) {
        try {
            LogUtil.log('LineChartRenderer drawData - start');
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.LineChartSetData);
            ...
            LogUtil.info('LineChartRenderer drawData succeed  ' + `lineData:${JSON.stringify(lineData)}`);
            LogUtil.info('LineChartRenderer drawData succeed  ' + `dataSets:${JSON.stringify(dataSets)}`);
        }
        catch (e) {
            LogUtil.error('LineChartRenderer drawData error', e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.LineChartSetData);
        }
        finally {
            LogUtil.log('LineChartRenderer drawData - end');
            MPChartTraceUtil.finish(TraceLogConstants.Tag.LineChartSetData);
        }
    }
    ...
    protected drawCubicFill(c: CanvasRenderingContext2D, dataSet: ILineDataSet, spline: Path2D, trans: Transformer, bounds: XBounds) {
        ...
        if (dataSet.getEntryCount() === 1) {
            ...
            spline.moveTo(entryIndex0.getX() - interval, entryIndex0.getY());
            spline.lineTo(entryIndex0.getX() + interval, entryIndex0.getY());
            spline.lineTo(entryIndex0.getX() + interval, fillMin);
            spline.lineTo(entryIndex0.getX() - interval, fillMin);
            spline.closePath();
        }
        else {
    ...
    protected drawLinear(c: CanvasRenderingContext2D, dataSet: ILineDataSet): void {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            for (let j = this.mXBounds.min; j < max; j++) {
                ...
                Utils.resetContext2DWithoutFont(canvas, this.mRenderPaint);
                canvas.beginPath();
     ...
     * @return
     */
    public generateFilledPath(dataSet: ILineDataSet, startIndex: number, endIndex: number, outputPath: Path2D): Path2D {
        if (!this.mChart) {
            return new Path2D();
        }
        const fillMin: number = dataSet.getFillFormatter().getFillLinePosition(dataSet, this.mChart);
            ...
            for (let i: number = 0; i < dataSets.size(); i++) {
                ...
                for (let j: number = 0; j < positions.length; j += 2) {
                    ...
                    if (dataSet.isDrawValuesEnabled()) {
                        ...
                        if (valueFormatter) {
                            this.drawValue(c, valueFormatter, entry.getY(), entry, i, x, y - valOffset, dataSet.getValueTextColor(j / 2), isHorizontalFlip);
                        }
                    }
    ...
    protected drawCircles(c: CanvasRenderingContext2D) {
        ...
        for (let i: number = 0; i < dataSets.size(); i++) {
            ...
            // 添加边界保护确保不超出数据范围
            const maxIndex: number = dataSet.getEntryCount() - 1;
            const safeEndIndex: number = Math.min(boundsRangeCount, maxIndex);
            const colorCount: number = dataSet.getCircleColorCount();
            for (let j: number = this.mXBounds.min; j <= safeEndIndex; j++) {
                const e: EntryOhos = dataSet.getEntryForIndex(j);
                // 同时检查null和undefined,防止访问无效Entry
                ...
                this.mRenderPaint.setColor(dataSet.getCircleColor(j % colorCount));
        ...
        if (drawTransparentCircleHole) {
            ...
            if (drawCircleHole) {
                if (this.mCirclePaintInner) {
                    Utils.resetContext2DWithoutFont(c, this.mCirclePaintInner);
                }
                c.beginPath();
    ...
    public drawHighlighted(c: CanvasRenderingContext2D, indices: Highlight[], isDraw?: boolean) {
        try {
            LogUtil.log("LineChartRenderer drawHighlighted - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.LineChartHighlight);
            if (!this.mChart || !isDraw) {
                return;
            }
            const lineData: LineData | null = this.mChart.getLineData();
            if (!lineData) {
            ...
            LogUtil.info("LineChartRenderer drawHighlighted succeed  " + `lineData: ${JSON.stringify(lineData)}`);
        }
        catch (e) {
            LogUtil.error('LineChartRenderer drawHighlighted error', e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.LineChartHighlight);
        }
        finally {
            LogUtil.log("LineChartRenderer drawHighlighted - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.LineChartHighlight);
        }
    }
    /**
...
class DataSetImageCache {
    mCirclePathBuffer = new Path2D();
    circleBitmaps: ImageBitmap[] | null = null;
    mCirclePaintInner: Paint = new Paint();
    init(set: ILineDataSet, circlePaintInner: Paint): boolean {
        this.mCirclePaintInner = circlePaintInner;
        const size: number = set.getCircleColorCount();
        let changeRequired: boolean = false;
        if (this.circleBitmaps == null) {
            this.circleBitmaps = new Array<ImageBitmap>(size);
            changeRequired = true;
        }
    ...
    public fill(set: ILineDataSet, drawCircleHole: boolean, drawTransparentCircleHole: boolean) {
        ...
        for (let i = 0; i < colorCount; i++) {
            let offscreenCanvas: OffscreenCanvasRenderingContext2D = new OffscreenCanvasRenderingContext2D(Math.floor(circleRadius * 2.1), Math.floor(circleRadius * 2.1));
            offscreenCanvas.fillStyle = set.getCircleColor(i); //设置填充颜色
            if (drawTransparentCircleHole) {
                //绘制带孔的圆
                this.mCirclePathBuffer = new Path2D();
                this.mCirclePathBuffer.arc(circleRadius, circleRadius, circleRadius, 0, Math.PI * 2, false);
                this.mCirclePathBuffer.arc(circleRadius, circleRadius, circleHoleRadius, 0, Math.PI * 2, true);
                offscreenCanvas.fill(this.mCirclePathBuffer);
            }
            else {
                //绘制完整的圆
                offscreenCanvas.beginPath();
                offscreenCanvas.arc(circleRadius, circleRadius, circleRadius, 0, Math.PI * 2, false);
                offscreenCanvas.fill();
                if (drawCircleHole) {
                    offscreenCanvas.beginPath();
                    offscreenCanvas.arc(circleRadius, circleRadius, circleHoleRadius, 0, Math.PI * 2, false);
                    offscreenCanvas.fillStyle = this.mCirclePaintInner.fillStyle;
                    offscreenCanvas.fill();
                }
            }
            if (this.circleBitmaps && this.circleBitmaps.length > 0) {
                this.circleBitmaps[i] = offscreenCanvas.transferToImageBitmap();
            }
        }
    }
    public getBitmap(index: number): ImageBitmap | null {
        if (this.circleBitmaps && this.circleBitmaps.length > 0) {
