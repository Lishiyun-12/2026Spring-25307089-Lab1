...
import type { DashPathEffect, FontFamily } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/data/Paint";
import { LegendDirection, LegendForm, LegendHorizontalAlignment, LegendOrientation, LegendVerticalAlignment } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/Legend";
import type Legend from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/Legend";
import LegendEntry from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/LegendEntry";
...
import type FSize from '../utils/FSize';
...
import Renderer from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/renderer/Renderer";
...
import type { AccessClick, LegendRect } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/utils/Constants";
export default class LegendRenderer extends Renderer {
    ...
    constructor(viewPortHandler: ViewPortHandler, legend: Legend) {
        ...
        this.mLegendLabelPaint.setTextSize(Utils.handleDataValues(9));
            ...
            for (let i: number = 0; i < data.getDataSetCount(); i++) {
                ...
                if (dataSet.constructor.name == "BarDataSet" && (dataSet as IBarDataSet).isStacked()) {
                    ...
                    for (let j: number = 0; j < minEntries; j++) {
                        ...
                        this.computedEntries.add(new LegendEntry(label, dataSet.getForm(), dataSet.getFormSize(), dataSet.getFormLineWidth(), dashEffect, clrs.get(j)?.valueOf() ?? 0));
                    }
                    ...
                    for (let j = 0; j < clrs.size() && j < entryCount; j++) {
                        this.computedEntries.add(new LegendEntry(pds.getEntryForIndex(j).getLabel(), dataSet.getForm(), dataSet.getFormSize(), dataSet.getFormLineWidth(), dataSet.getFormLineDashEffect(), clrs.get(j)?.valueOf() ?? 0));
                    }
    ...
    public renderLegend(c: CanvasRenderingContext2D): void {
        try {
            LogUtil.log("LegendRenderer renderLegend - start");
            ...
            this.mLegendLabelPaint.setColor(this.mLegend.getTextColor());
            // 计算标签行高
            let labelLineHeight: number = Utils.getLabelTextSize(this.mLegendLabelPaint, "Text", this.textSizeCache).height;
            // 计算标签行间距，包括额外的垂直空间
            let labelLineSpacing: number = labelLineHeight * 0.3 + Utils.handleDataValues(this.mLegend.getYEntrySpace());
            // 计算形状偏移以使其与文本垂直居中
            let formYOffset: number = labelLineHeight - labelLineHeight / 2.0;
            // 获取图例条目
            let entries: LegendEntry[] = this.mLegend.getEntries();
            // 获取形状到文本的间距
            let formToTextSpace: number = Utils.handleDataValues(this.mLegend.getFormToTextSpace());
            // 获取水平条目之间的空间
            let xEntrySpace: number = Utils.handleDataValues(this.mLegend.getXEntrySpace());
            // 获取图例方向
            ...
            // 获取默认形状的大小
            let defaultFormSize: number = Utils.handleDataValues(this.mLegend.getFormSize());
            // 条目之间的空间
            let stackSpace: number = Utils.handleDataValues(this.mLegend.getStackSpace());
            // 获取垂直偏移量
            ...
            switch (horizontalAlignment) {
                case LegendHorizontalAlignment.LEFT:
                    ...
                    if (this.mViewPortHandler) {
                        LogUtil.log("1. originPosX(" + JSON.stringify(originPosX) + ")= xoffset(" + xoffset + ") + mViewPortHandler.contentLeft(" + this.mViewPortHandler.contentLeft() + ")");
                    }
                ...
                case LegendOrientation.HORIZONTAL: {
                    ...
                    switch (verticalAlignment) { // "BOTTOM"
                        case LegendVerticalAlignment.TOP:
                            posY = yoffset;
                            LogUtil.log("2.0 posY(" + JSON.stringify(posY) + ")= yoffset(" + yoffset + ")");
                            break;
                        case LegendVerticalAlignment.BOTTOM:
                            if (this.mViewPortHandler) {
                                posY = this.mViewPortHandler.getChartHeight() - yoffset - this.mLegend.mNeededHeight;
                                LogUtil.log("2.1 posY(" + posY + ")= mViewPortHandler.getChartHeight(" + this.mViewPortHandler.getChartHeight() + ")-mNeededHeight(" + this.mLegend.mNeededHeight + ")-yoffset(" + yoffset + ")");
                            }
                            break;
                        case LegendVerticalAlignment.CENTER:
                            if (this.mViewPortHandler) {
                                posY = (this.mViewPortHandler.getChartHeight() - this.mLegend.mNeededHeight) / 2.0 + yoffset;
                                LogUtil.log("2.1 posY(" + posY + ")= mViewPortHandler.getChartHeight(" + this.mViewPortHandler.getChartHeight() + ")-mNeededHeight/2(" + this.mLegend.mNeededHeight / 2 + ")+yoffset(" + yoffset + ")");
                            }
                            break;
                    }
                    ...
                    for (let i: number = 0, count = entries.length; i < count; i++) {
                        ...
                        let formSize: number = Number.isNaN(e.formSize) ? defaultFormSize : Utils.handleDataValues(e.formSize);
                        if (i < calculatedLabelBreakPoints.size() && calculatedLabelBreakPoints.get(i)) {
                            posX = originPosX;
                            posY += labelLineHeight + labelLineSpacing;
                            LogUtil.log("3.0 i(" + i + ") posX=" + posX + "; posY(" + posY + ")=labelLineHeight(" + labelLineHeight + ")+labelLineSpacing(" + labelLineSpacing + ")");
                        }
                        if (posX == originPosX &&
                        ...
                        if (drawingForm) {
                            ...
                            LogUtil.log(`drawForm posX = ${posX}`);
                            if (direction == LegendDirection.LEFT_TO_RIGHT) { // "LEFT_TO_RIGHT"
                                LogUtil.log("3.2 i(" + i + ") posX(" + posX + ")+=formSize(" + formSize + ")");
                                posX += formSize;
                            }
                        }
                        if (!isStacked) {
                            if (drawingForm) {
                                LogUtil.log("3.3 i(" + i + ") posX(" + posX + ")+=formToTextSpace(" + formToTextSpace + ")");
                            ...
                            let isAccess = AppStorage.get(Constants.LEGEND_ACCESS_BOOL) as boolean;
                            if (isAccess) {
                                let accessClick: AccessClick = AppStorage.get(Constants.LEGEND_ACCESS) as AccessClick;
                                let rectItem: LegendRect = { x: posX - formSize, y: posY, width: c.measureText(e.label).width + defaultFormSize + formToTextSpace, height: c.measureText(e.label).height };
                                let rectList: Array<LegendRect> = [];
                                rectList.push(rectItem);
                                // 遍历所有矩形，检查点击位置是否在其中
                                for (let i = 0; i < rectList.length; i++) {
                                    const rect = rectList[i];
                                    // 矩形命中检测
                                    if (accessClick.x >= rect.x && accessClick.x <= rect.x + rect.width &&
                                        accessClick.y >= rect.y && accessClick.y <= rect.y + rect.height) {
                                        this.drawBorder(c, rect.x, rect.y, rect.width, rect.height);
                                        AppStorage.setOrCreate(Constants.LEGEND_ACCESS_LABEL, e.label);
                                        AppStorage.setOrCreate(Constants.LEGEND_ACCESS_BOOL, false);
                                    }
                                    else {
                                        this.drawBorder(c, 0, 0, 0, 0);
                                        if (isAccess) {
                                            AppStorage.setOrCreate(Constants.LEGEND_ACCESS_LABEL, 'No section selected');
                                        }
                                    }
                                }
                            }
                            LogUtil.log(`drawLabel posX = ${posX}, e.label = ${e.label}}`);
                            if (direction == LegendDirection.LEFT_TO_RIGHT) {
                                posX += calculatedLabelSizes.get(i).width;
                            }
                            LogUtil.log("3.6 i(" + i + ") posX(" + posX + ")+=xEntrySpace(" + xEntrySpace + "); stackSpace(" + stackSpace + ")");
                    ...
                    for (let i: number = 0; i < entries.length; i++) {
                        ...
                        if (e.label != null && e.label != "") {
                            if (drawingForm && !wasStacked)
                                ...
                                posX = originPosX;
                            if (direction == LegendDirection.RIGHT_TO_LEFT) {
                                posX -= Utils.getLabelTextSize(this.mLegendLabelPaint, e.label, this.textSizeCache).width;
                            }
                            if (!wasStacked) {
                                this.drawLabel(c, posX, posY + labelLineHeight, e.label);
                                let isAccess = AppStorage.get(Constants.LEGEND_ACCESS_BOOL) as boolean;
            ...
            LogUtil.error("LegendRenderer renderLegend error", e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.renderLegend);
        }
        finally {
            LogUtil.log("LegendRenderer renderLegend - end");
    ...
    protected drawForm(c: CanvasRenderingContext2D, x: number, y: number, entry: LegendEntry, legend: Legend): void {
        ...
        let formSize: number = Utils.handleDataValues(Number.isNaN(entry.formSize)
            ? legend.getFormSize()
            : entry.formSize);
        ...
        switch (form) {
            ...
            case LegendForm.CIRCLE:
                this.mLegendFormPaint.setStyle(Style.FILL);
                c.fillStyle = this.mLegendFormPaint.fillStyle;
                c.beginPath();
            ...
            case LegendForm.LINE:
                {
                    let formLineWidth: number = Utils.handleDataValues(Number.isNaN(entry.formLineWidth)
                    ...
                    Utils.resetContext2DWithoutFont(c, this.mLegendFormPaint);
                    c.beginPath();
                    c.moveTo(x, y);
    ...
    protected drawBorder(c: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): void {
        c.strokeStyle = '#6bc843';
        c.lineWidth = 2;
