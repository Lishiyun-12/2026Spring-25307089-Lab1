...
import Paint, { Style, TextPaint } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/data/Paint";
import type PieData from '../data/PieData';
import { ValuePosition } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/data/PieDataSet";
import type PieEntry from '../data/PieEntry';
...
import MyMath from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/utils/MyMath";
...
export default class PieChartRenderer extends DataRenderer {
     ...
     * chart
     */
    protected mCenterTextPaint: TextPaint;
    /**
     * paint object used for drwing the slice-text
     */
    protected mEntryLabelsPaint: Paint;
    ...
    constructor(chart: PieChart, animator: ChartAnimator, viewPortHandler: ViewPortHandler) {
        ...
        this.mCenterTextPaint.setTextSize(Utils.handleDataValues(12));
        this.mValuePaint.setTextSize(Utils.handleDataValues(13));
        ...
        this.mEntryLabelsPaint.setTextSize(Utils.handleDataValues(13));
    ...
    public drawData(c: CanvasRenderingContext2D): void {
        try {
            LogUtil.log('PieChartRenderer drawData - start');
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.PieChartSetData);
            ...
            LogUtil.info('PieChartRenderer drawData succeed  ' + `pieData:${JSON.stringify(pieData)}`);
        }
        catch (e) {
            LogUtil.error('PieChartRenderer drawData error', e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.PieChartSetData);
        }
        finally {
            LogUtil.log('PieChartRenderer drawData - end');
            MPChartTraceUtil.finish(TraceLogConstants.Tag.PieChartSetData);
        }
    }
    ...
    public drawValues(c: CanvasRenderingContext2D): void {
        try {
            LogUtil.log("PieChartRenderer drawValues - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.PieChartDrawValues);
            if (!this.mAnimator) {
                return;
            }
            ...
            let xIndex: number = 0;
            c.save();
            let offset: number = Utils.handleDataValues(0.0);
            const lineHeight: number = Utils.getLabelTextSize(this.mValuePaint, "Q", this.textSizeCache)
                .height + Utils.handleDataValues(4);
            for (let i: number = 0; i < dataSets.size(); i++) {
                ...
                this.mValueLinePaint.setStrokeWidth(Utils.handleDataValues(dataSet.getValueLineWidth()));
                ...
                for (let j: number = 0; j < entryCount; j++) {
                    ...
                    if (drawXOutside || drawYOutside) {
                        ...
                        if (lineColor != ColorTemplate.COLOR_NONE) {
                            ...
                            Utils.resetContext2DWithoutFont(c, this.mValueLinePaint);
                            c.beginPath();
                            c.moveTo(pt0x, pt0y);
                            c.lineTo(pt1x, pt1y);
                            c.lineTo(pt2x, pt2y);
                    ...
                    if (entry.getIcon() != null && dataSet.isDrawIconsEnabled()) {
                        ...
                        y += iconsOffset.x;
                        if (icon) {
                            Utils.drawImage(c, icon, x, y, icon.getWidth(), icon.getHeight());
                        }
                    }
                    xIndex++;
                }
            ...
            LogUtil.error('PieChartRenderer drawValues error', e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.PieChartDrawValues);
        }
        finally {
            LogUtil.log("PieChartRenderer drawValues - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.PieChartDrawValues);
        }
    }
    /**
    ...
    protected drawEntryLabel(c: CanvasRenderingContext2D, label: string, x: number, y: number): void {
        ...
        Utils.resetContext2DWithoutLine(c, this.mEntryLabelsPaint);
        c.beginPath();
    ...
    protected drawHole(c: CanvasRenderingContext2D): void {
        try {
            LogUtil.log("PieChartRenderer drawHole - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.PieChartDrawHole);
            if (this.mChart.isDrawHoleEnabled()) {
                let radius: number = this.mChart.getRadius();
            ...
            LogUtil.error('PieChartRenderer drawHole error', e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.PieChartDrawHole);
        }
        finally {
            LogUtil.log("PieChartRenderer drawHole - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.PieChartDrawHole);
        }
    }
    ...
    protected drawCenterText(c: CanvasRenderingContext2D): void {
        try {
            LogUtil.log("PieChartRenderer drawCenterText - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.PieChartDrawCenterText);
            ...
            if (this.mChart.isDrawCenterTextEnabled() && centerText != null) {
                ...
                let textWidth: number = Utils.getLabelTextSize(this.mCenterTextPaint, centerText, this.textSizeCache).width;
                c.save();
                Utils.resetContext2DWithoutLine(c, this.mCenterTextPaint);
                c.fillText(centerText, x - textWidth / 2, y);
                c.restore();
                MPPointF.recycleInstance(center);
                MPPointF.recycleInstance(offset);
            }
            LogUtil.info("PieChartRenderer drawCenterText succeed  " + `centerText:${JSON.stringify(centerText)}`);
        }
        catch (e) {
            LogUtil.error('PieChartRenderer drawCenterText error', e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.PieChartDrawCenterText);
        }
        finally {
            LogUtil.log("PieChartRenderer drawCenterText - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.PieChartDrawCenterText);
        }
    }
    ...
    public drawHighlighted(c: CanvasRenderingContext2D, indices: Highlight[]): void {
        let isAccess: boolean = AppStorage.get(Constants.PIE_CHART_ACCESS) as boolean;
        ...
        for (let i: number = 0; i < indices.length; i++) {
            ...
            let highlightedRadius: number = 0;
            if (isAccess) {
                ...
                highlightedRadius = radius + shift;
                highlightedCircleBox.set(this.mChart.getCircleBox().left, this.mChart.getCircleBox()
                    .top, this.mChart.getCircleBox().right, this.mChart.getCircleBox().bottom);
            ...
            if (sweepAngleOuter >= 360.0 && sweepAngleOuter % 360 <= Utils.FLOAT_EPSILON) {
                // ohos is doing "mod 360"
                this.mPathBuffer.arc(center.x, center.y, highlightedRadius, 0, 2 * Math.PI, false);
            }
            else {
    ...
    protected drawRoundedSlices(c: CanvasRenderingContext2D): void {
        try {
            LogUtil.log("PieChartRenderer drawRoundedSlices - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.PieChartDrawRoundedSlices);
            ...
            MPPointF.recycleInstance(center);
            LogUtil.info("PieChartRenderer drawRoundedSlices succeed  " + `phaseX:${JSON.stringify(phaseX)} ` + `phaseY:${JSON.stringify(phaseY)}`);
        }
        catch (e) {
            LogUtil.error('PieChartRenderer drawRoundedSlices error', e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.PieChartDrawRoundedSlices);
        }
        finally {
            LogUtil.log("PieChartRenderer drawRoundedSlices - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.PieChartDrawRoundedSlices);
        }
    }
    /**
