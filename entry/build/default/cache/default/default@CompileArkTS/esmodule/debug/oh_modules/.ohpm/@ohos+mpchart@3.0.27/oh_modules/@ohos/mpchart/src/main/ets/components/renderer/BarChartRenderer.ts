...
import type Range from '../highlight/Range';
...
import { FillDirection } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/utils/Fill";
import type Fill from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/utils/Fill";
...
import type { RoundCornerType } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/utils/Utils";
...
export default class BarChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    public drawData(c: CanvasRenderingContext2D): void {
        try {
            LogUtil.log("BarChartRenderer drawData - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.BarChartSetData);
            if (!this.mChart) {
                return;
            }
            let barData: BarData | null = this.mChart.getBarData();
            ...
            LogUtil.info("BarChartRenderer drawData succeed  " + `barData:${JSON.stringify(barData)}`);
        }
        catch (e) {
            MPChartTraceUtil.startError(TraceLogConstants.Tag.BarChartSetData);
            LogUtil.error("BarChartRenderer drawData error", e);
        }
        finally {
            MPChartTraceUtil.finish(TraceLogConstants.Tag.BarChartSetData);
            LogUtil.log("BarChartRenderer drawData - end");
        }
    }
    ...
    protected drawDataSet(c: CanvasRenderingContext2D, dataSet: IBarDataSet, index: number): void {
        ...
        for (let j = 0, pos = 0; j < buffer.size(); j += 4, pos++) {
            ...
            if (isCustomFill) {
                ...
                if (fill) {
                    fill.fillRect(c, this.mRenderPaint, buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2], buffer.buffer[j + 3], isInverted ? FillDirection.DOWN : FillDirection.UP, radius, cornerType);
                }
            }
            else {
                ...
                if (!dataSet.isStacked()) {
                    for (let j = 0; j < buffer.buffer.length * (this.mAnimator ? this.mAnimator.getPhaseX() : 1); j += 4) {
                        ...
                        if (icon && dataSet.isDrawIconsEnabled()) {
                            ...
                            px += iconsOffset.x;
                            py += iconsOffset.y;
                            Utils.drawImage(c, icon, Utils.parseInt(px), Utils.parseInt(py), icon.getWidth(), icon.getHeight());
                        }
                    }
                    // if we have stacks
                }
                    ...
                    while (index < dataSet.getEntryCount() * (this.mAnimator ? this.mAnimator.getPhaseX() : 1)) {
                        ...
                        if (vals == null) {
                            ...
                            for (let k = 0; k < transformed.length; k += 2) {
                                ...
                                let icon: ChartPixelMap | null = entry.getIcon();
                                if (icon && dataSet.isDrawIconsEnabled()) {
                                    Utils.drawImage(c, icon, Number(x + iconsOffset.x), Number(y + iconsOffset.y), icon.getWidth(), icon.getHeight());
                                }
                            }
                        }
    ...
    public drawHighlighted(c: CanvasRenderingContext2D, indices: Highlight[]): void {
        try {
            LogUtil.log("BarChartRenderer drawHighlighted - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.BarChartHighlight);
            if (!this.mChart) {
                return;
            }
            let barData: BarData | null = this.mChart.getBarData();
            ...
            for (let i = 0; i < indices.length; i++) {
                ...
                let isAccess: boolean = AppStorage.get(Constants.BAR_CHART_ACCESS) as boolean;
                if (isAccess) {
                    if (this.mBarBorderPaint != null) {
                        this.mBarBorderPaint.setColor('#6bc843');
                        this.mBarBorderPaint.setStyle(Style.STROKE);
                        this.mBarBorderPaint.setStrokeWidth(3);
                        Utils.drawStrokeRect(drawRadius, c, this.mBarBorderPaint, this.mBarRect.left, this.mBarRect.top, this.mBarRect.right, this.mBarRect.bottom, radius, cornerType);
                    }
                }
                else {
            ...
            LogUtil.info("BarChartRenderer drawHighlighted succeed  " + `barData:${JSON.stringify(barData)}`);
        }
        catch (e) {
            MPChartTraceUtil.startError(TraceLogConstants.Tag.BarChartHighlight);
            LogUtil.error("BarChartRenderer drawHighlighted error", e);
        }
        finally {
            MPChartTraceUtil.finish(TraceLogConstants.Tag.BarChartHighlight);
            LogUtil.log("BarChartRenderer drawHighlighted - end");
        }
    }
    /**
