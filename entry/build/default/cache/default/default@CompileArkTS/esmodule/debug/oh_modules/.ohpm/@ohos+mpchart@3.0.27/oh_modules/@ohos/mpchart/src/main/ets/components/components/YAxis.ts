import AxisBase from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/AxisBase";
import type { LabelInfo } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/AxisBase";
...
export default class YAxis extends AxisBase {
    ...
    /**
     * the minimum width that the axis should take (in vp).
     * <p/>
     * default: 0.0
     */
    protected mMinWidth: number = 0;
    /**
     * the maximum width that the axis can take (in vp).
     ...
     */
    private customYAxisLabels: number[] = [];
    /**
     * Is it a fully customized y-axis label
     */
    private enableCustomYAxisLabels: boolean = false;
    private mLabelYInfos: LabelInfo[] = [];
    ...
    public setLabelYInfos(labelInfos: LabelInfo[]) {
        this.mLabelYInfos = labelInfos;
    }
    public getLabelYInfos(): LabelInfo[] {
        return this.mLabelYInfos;
    }
    /**
     * Set Y-axis custom labels
     * @param numbers labels
     */
    public setCustomYAxisLabels(numbers: number[]): void {
        this.customYAxisLabels = numbers;
    }
    /**
    ...
    public getCustomYAxisLabels(): number[] {
        return this.customYAxisLabels;
    }
    /**
     * Delete Y-axis custom labels
     * @param numbers labels
     */
        ...
        if (numbers != null && numbers != undefined) {
            ...
            for (let i: number = 0; i < this.customYAxisLabels.length; i++) {
                ...
                for (let index: number = 0; index < numbers.length; index++) {
                    if (this.customYAxisLabels[i] == numbers[index]) {
                        flag = true;
                        break;
                    }
                }
                if (!flag) {
                    YAxisLabels.push(this.customYAxisLabels[i]);
                }
            }
            this.customYAxisLabels = YAxisLabels;
        }
    }
    /**
    ...
    public setEnableCustomYAxisLabels(enableCustomYAxisLabels: boolean): void {
        this.enableCustomYAxisLabels = enableCustomYAxisLabels;
    }
    /**
     *  Get whether to fully use custom labels
     * @param numbers labels
     */
    public getEnableCustomYAxisLabels(): boolean {
        return this.enableCustomYAxisLabels;
    }
    ...
    /**
     * Sets the maximum width that the axis can take (in vp).
     *
     * @param maxWidth
     */
     ...
     * @param enabled
     */
    public setDrawBottomYLabelEntry(enabled: boolean): void {
    ...
    /**
     * Sets the width of the zero line in vp
     *
     * @param width
     */
    ...
    public getRequiredWidthSpace(p: Paint): number {
        ...
        let width: number = Utils.getLabelTextSize(p, label, this.textSizeCache).width + super.getXOffset() * 2;
        let minWidth: number = this.getMinWidth();
        let maxWidth: number = this.getMaxWidth();
        if (minWidth > 0)
            minWidth = Utils.handleDataValues(minWidth);
        if (maxWidth > 0 && maxWidth != Number.POSITIVE_INFINITY)
            maxWidth = Utils.handleDataValues(maxWidth);
