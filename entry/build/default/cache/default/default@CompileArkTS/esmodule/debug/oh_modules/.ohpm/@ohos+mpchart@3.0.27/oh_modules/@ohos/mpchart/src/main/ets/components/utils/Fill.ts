...
import ChartPixelMap from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/data/ChartPixelMap";
...
export default class Fill {
    ...
    public async fillRect(c: CanvasRenderingContext2D, paint: Paint, left: number, top: number, right: number, bottom: number, gradientDirection: FillDirection, radius?: number, cornerType: RoundCornerType = 'up'): Promise<void> {
        switch (this.mType) {
            case FillType.EMPTY:
                return;
            case FillType.COLOR:
    ...
    public async fillPath(c: CanvasRenderingContext2D, path: Path2D, paint: Paint, clipRect: MyRect): Promise<void> {
