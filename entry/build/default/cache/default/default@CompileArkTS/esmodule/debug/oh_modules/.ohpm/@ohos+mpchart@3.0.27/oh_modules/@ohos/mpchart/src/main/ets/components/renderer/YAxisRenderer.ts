...
import { AxisDependency, YAxisLabelPosition } from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/YAxis";
import type YAxis from "@package:pkg_modules/.ohpm/@ohos+mpchart@3.0.27/pkg_modules/@ohos/mpchart/src/main/ets/components/components/YAxis";
...
export default class YAxisRenderer extends AxisRenderer {
    ...
    public renderAxisLabels(c: CanvasRenderingContext2D, isHorizontalFlip: boolean, isLeftAxis: boolean): void {
        try {
            LogUtil.log('YAxisRenderer renderAxisLabels - start');
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.renderYAxisLabels);
            if (!this.mYAxis) {
                return;
            }
            ...
            MPChartTraceUtil.startError(TraceLogConstants.Tag.renderYAxisLabels);
            LogUtil.error("YAxisRenderer renderAxisLabels error", e);
        }
        finally {
            LogUtil.log('YAxisRenderer renderAxisLabels - end');
            MPChartTraceUtil.finish(TraceLogConstants.Tag.renderYAxisLabels);
        }
        // return [];
    }
    public renderAxisLine(c: CanvasRenderingContext2D, extraLength: number): void {
        try {
            LogUtil.log('YAxisRenderer renderAxisLine - start');
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.renderYAxisLine);
            ...
            MPChartTraceUtil.startError(TraceLogConstants.Tag.renderYAxisLine);
            LogUtil.error("YAxisRenderer renderAxisLine error", e);
        }
        finally {
            LogUtil.log('YAxisRenderer renderAxisLine - end');
            MPChartTraceUtil.finish(TraceLogConstants.Tag.renderYAxisLine);
        }
    }
    /**
    ...
    protected drawYLabels(c: CanvasRenderingContext2D, fixedPosition: number, positions: number[], offset: number, isHorizontalFlip: boolean, isLeftAxis: boolean): void {
                    ...
                    if (isHorizontalFlip) {
                        if (isLeftAxis) {
                            ...
                            labelInfo = {
                                ...
                                y: positions[i * 2 + 1] + offset - textHeight,
                                width: textWidth,
                                height: textHeight,
                                type: 'y'
                            };
                ...
                default:
                    ...
                    if (isHorizontalFlip) {
                        ...
                        labelInfo = {
                            value: text,
                            x: xPosition - textWidth,
                            y: positions[i * 2 + 1] - textHeight / 2,
                            width: textWidth,
                            height: textHeight,
                            type: 'y'
                        };
        ...
        this.mYAxis.setLabelYInfos(labelInfos);
        // return paints;
    }
    ...
    public renderGridLines(c: CanvasRenderingContext2D): void {
        try {
            ...
            //   ? this.mYAxis.mEntryCount
            //   : (this.mYAxis.mEntryCount - 1);
            LogUtil.log('YAxisRenderer renderGridLines - start');
            ...
            LogUtil.error("YAxisRenderer renderGridLines error", e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.renderGridLines);
        }
        finally {
            LogUtil.log('YAxisRenderer renderGridLines - end');
    ...
    public renderLimitLines(c: CanvasRenderingContext2D, isHorizontalFlip: boolean): void {
        try {
            LogUtil.log("YAxisRenderer renderLimitLines - start");
            MPChartTraceUtil.startInfo(TraceLogConstants.Tag.renderYAxisLimitLine);
            if (!this.mYAxis) {
                return;
            }
            ...
            LogUtil.info("YAxisRenderer renderLimitLines succeed  " + `limitLines:${JSON.stringify(limitLines)}`);
        }
        catch (e) {
            LogUtil.error('YAxisRenderer renderLimitLines error', e);
            MPChartTraceUtil.startError(TraceLogConstants.Tag.renderYAxisLimitLine);
        }
        finally {
            LogUtil.log("YAxisRenderer renderLimitLines - end");
            MPChartTraceUtil.finish(TraceLogConstants.Tag.renderYAxisLimitLine);
        }
    }
