...
// 构造数据选择监听器
  ...
  aboutToAppear() {
    ...
    // {x}-坐标中x值, {y}-坐标中y值, {legend}-图例内容, {axis}-x/y轴, {value}-轴体的值
    // 设置朗读模式图表内容朗读
...
```
  ...
  aboutToAppear() {
    // Step2：配置图表的特定样式，各部件间没有先后之分
     ...
     // {label}-扇形块名称, {value}-扇形块的值, {legend}-图例内容
    // 设置朗读模式图表内容朗读
    this.model.setReadChartText('select: {label}, value: {value}');
    // 设置朗读模式图例朗读模版
    this.model.setReadLegendText('legend: {legend}');
...
RoseData            // 玫瑰图数据包
  ...
  // Step1:必须：初始化图表配置构建类
  model: RoseChartModel = new RoseChartModel();
  ...
  // Step2：配置图表的特定样式
  @State options: RoseOption = {
    title: {
      value: 'Nightingale Chart',   // 标题内容
      titleStyle: { fontSize: '50px' } // 标题样式
    },
    center: ['50%', '50%'], // 图标位置
    dataView: true, // 是否显示每项数据的文本
    showValue: false,  // 是否显示每项数据的值
    radius: [20, 100], // 【中心孔的大小， 外圆弧最大半径】
    description: 'right', // 视图示例显示的位置
    roseType: 'radius', // 数据显示模式  'radius'半径显示数据值的大小、 'area'面积大小显示数值的大小
    roseData: this.resetData(8), // 设置图表数据
  };
  
  // 初始化玫瑰图数据
  public resetData(num: number) {
    let data: SectorData[] = [];
    for (let i = 0; i < num; i++) {
      let random_number: number = Math.floor(Math.random() * 81) + 20;
      data.push({
        value: random_number, // 数据值的大小
        label: `part${i + 1}` // 数据label文本
        color?: number | string; // 设置数据扇形颜色
        countingUnit?: string;  // 设置数据单位
      });
    }
    return data;
  }
  ...
  // 生成图标数据
  const roseData: RoseData = new RoseData(this.options);
  // 将数据与图标配置绑定
  this.model.setData(roseData);
```
...
```
  ...
  aboutToAppear() {
    ...
    // Step2：配置图表的特定样式，各部件间没有先后之分
     ...
     // {x}-坐标中x值, {y}-坐标中y值, {size}-面积大小, {legend}-图例内容, {axis}-x/y轴, {value}-轴体的值
    // 设置朗读模式图表内容朗读
    this.model.setReadChartText('x轴: {x}, y轴: {y}, 面积: {size}');
    // 设置朗读模式图例朗读模版
    this.model.setReadLegendText('legend: {legend}');
    // 设置朗读模式坐标轴朗读模版
    this.model.setReadAxisText('{axis}轴: {value}');
...
```
  ...
  aboutToAppear() {
    ...
    // {x}-坐标中x值, {y}-坐标中y值, {max}-最大值, {min}-最小值, {open}-开盘值, {close}-收盘值
    // {legend}-图例内容, {axis}-x/y轴, {value}-轴体的值
    // 设置朗读模式图表内容朗读
    this.model.setReadChartText('x轴: {x}, y轴: {y}, maxValue: {max}, minValue: {min}, openValue: {open}, closeValue: {close}');
    // 设置朗读模式图例朗读模版
    this.model.setReadLegendText('legend: {legend}');
    // 设置朗读模式坐标轴朗读模版
    this.model.setReadAxisText('{axis}轴: {value}');
...
```
   ...
   aboutToAppear() {
    ...
    // {value}-轴体的值
    // 设置朗读模式图表内容朗读
    this.model.setReadChartText('value: {value}');
...
```
 import {
  ...
  aboutToAppear() {
    ...
    // {x}-坐标中x值, {y}-坐标中y值, {min}-最小值, {max}-最大值, {legend}-图例内容, {axis}-x/y轴, {value}-轴体的值
    // 设置朗读模式图表内容朗读
    this.model.setReadChartText('x轴: {x}, y轴: {y}, minValue: {min}, maxValue: {max}');
    // 设置朗读模式图例朗读模版
    this.model.setReadLegendText('legend: {legend}');
    // 设置朗读模式坐标轴朗读模版
    this.model.setReadAxisText('{axis}轴: {value}');
...
```
 import {
  FunnelOption,          // 漏斗图配置
  FunnelChart,           // 漏斗图组件
  FunnelChartModel,      // 漏斗图配置构建类
 } from '@ohos/mpchart';
 
  // Step1:必须：初始化图表配置构建类
 this.model = new FunnelChartModel();
 
 // Step2:设置图表配置项，所有配置项均为可选参数，如不传则采用默认配置
 
 // {type}-类型, {title}-标题, {name}-名称
 // 设置朗读模式图表内容朗读
 this.model.setReadChartText('type: {type}, title: {title}, name: {name}');
 // 设置朗读模式其他朗读模版
 this.model.setReadOtherText('type: {type}, title: {title}');
 
 // 漏斗图图形层级为最高，title和legend需设置参数调整合适位置，保证不被图形覆盖
 this.model.setOption({
   title:{                             // 漏斗图标题配置
    show: true,                        // 漏斗图标题是否显示
    text: 'text',                      // 漏斗图主标题文本
    subtext: 'subtext',                // 漏斗图副标题文本
    left: 'center',                    // 漏斗图标题离容器左侧的距离，left 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'left', 'center', 'right'
    top: 'top',                        // 漏斗图标题离容器上侧的距离，top 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'top', 'center', 'bottom'
    backgroundColor: 'rgba(0,0,0,0)',  // 漏斗图标题背景颜色
    itemGap: 10,                       // 漏斗图主副标题间距
    textColor: Color.Black,            // 漏斗图主标题文本颜色
    textFont: 16,                      // 漏斗图主标题文本字体大小
    subtextColor: Color.Gray,          // 漏斗图副标题文本颜色
    subtextFont: 14                    // 漏斗图副标题文本字体大小
   },
   legend:{                            // 漏斗图图例配置
    show: true,                        // 漏斗图图例是否显示
    orient: 'horizontal',              // 漏斗图图例列表的布局朝向，'horizontal' | 'vertical'
    gap: 8,                        // 漏斗图图例每项之间的间隔
    itemType: 'DEFAULT',               // 漏斗图图例图形样式，'SQUARE' | 'CIRCLE' | 'LINE' | 'DEFAULT' | 'EMPTY'
    itemWidth: 25,                     // 漏斗图图例图形宽度
    itemHeight: 14,                    // 漏斗图图例图形高度
    itemGap: 8,                        // 漏斗图图例图形和文字的间隔
    left: 'center',                    // 漏斗图图例离容器左侧的距离，left 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'left', 'center', 'right'
    top: 'bottom',                     // 漏斗图图例离容器上侧的距离，top 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'top', 'center', 'bottom'
    backgroundColor: 'rgba(0,0,0,0)',  // 漏斗图图例背景颜色
    textFont: 12,                      // 漏斗图图例文本字体大小
    textColor: Color.Black,            // 漏斗图图例文本颜色
    data: ['xxx1','xxx2']              // 漏斗图图例数据数组
   },
   tooltip: {                          // 漏斗图提示框配置
    show: true,                        // 漏斗图提示框是否显示
    triggerOn: 'click',                // 漏斗图提示框触发方式，'mousemove' | 'click'
    backgroundColor: '#FFFFFF',        // 漏斗图提示框触背景色
    formatter: '{a}\n{b}:{c}',         // 漏斗图提示框内容格式器，{a}（系列名称），{b}（文字名称），{c}（数值）
    borderRadius: 4,                   // 漏斗图提示框边框圆角
    borderWidth: 1,                    // 漏斗图提示框边框宽度
    borderColor: '#000000',            // 漏斗图提示框边框颜色
    textColor: '#000000',              // 漏斗图提示框内容文字颜色
    textFont: 14                       // 漏斗图提示框内容文字字体大小
   },
   series:[
    {
      name: 'xxx',                     // 漏斗图系列名称
      width: 0,                        // 漏斗图系列宽度，width 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比
      height: 0,                       // 漏斗图系列高度，height 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比
      left: 10,                        // 漏斗图系列离容器左侧的距离，left 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'left', 'center', 'right'
      top: 30,                         // 漏斗图系列离容器上侧的距离，top 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'top', 'center', 'bottom'
      bottom: 60,                      // 漏斗图系列离容器下侧的距离，bottom 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'top', 'center', 'bottom'
      right: 10,                       // 漏斗图系列离容器右侧的距离，right 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'left', 'center', 'right'
      funnelAlign: 'center',           // 漏斗图系列水平方向对齐布局类型，默认居中对齐，可用选项还有：'left' | 'right' | 'center'
      sort: 'descending',              // 漏斗图系列数据排序， 可以取 'ascending'，'descending'，'none'（表示按 data 顺序）
      gap: 2,                          // 漏斗图系列数据图形间距
      label: {                         // 漏斗图系列图形上的文本标签
        show: true,                    // 漏斗图系列图形上的文本标签是否显示
        position: 'inside',            // 漏斗图系列图形上的文本标签位置
        textColor: Color.Black,        // 漏斗图系列图形上的文本标签颜色
        textFont: 16,                  // 漏斗图系列图形上的文本标签字体大小
        line: {                        // 漏斗图系列标签视觉引导线样式
          show: true,                  // 漏斗图系列标签视觉引导线是否显示
          length: 10,                  // 漏斗图系列标签视觉引导线长度
          width: 1,                    // 漏斗图系列标签视觉引导线宽度
          color: Color.Black           // 漏斗图系列标签视觉引导线颜色
        },
      },
      data: [                          // 漏斗图系列数据集合
        {
...
// 为组件设置配置构建类，如果需要在页面初始化就显示图表，则需要在aboutToAppear方法中完成图表数据构建
// 如果在之后通过事件触发，
// 可通过this.model.setOption();来触发数据更新，
// 可通过this.model.invalidate();来触发绘制更新。
FunnelChart({ model: this.model })
.width('100%')
.height('70%')
```
...
```
 import {
  WordCloudOption,      // 词云图配置
  WordCloudChart,       // 词云图组件
  WordCloudChartModel,  // 词云图配置构建类
  WordCloudType,        // 词云图图案类型
 } from '@ohos/mpchart';
 
 // Step1:必须：初始化图表配置构建类
 this.model = new WordCloudChartModel();
 
 // Step2:设置图表配置项，所有配置项均为可选参数，如不传则采用默认配置
 
 // {title}-标题, {name}-名称, {value}-值
 // 设置朗读模式图表内容朗读
 this.model.setReadChartText('title: {title}, name: {name}, value: {value}');
 
 this.model.setOption({
   tooltip: {                       // 词云图提示框配置
     show: true,                    // 词云图提示框是否显示
     triggerOn: 'click',            // 词云图提示框触发方式，'mousemove' | 'click'
     backgroundColor: '#FFFFFF',    // 词云图提示框触背景色
     formatter: '{a}\n{b}:{c}',     // 词云图提示框内容格式器，{a}（系列名称），{b}（文字名称），{c}（数值）
     borderRadius: 4,               // 词云图提示框边框圆角
     borderWidth: 1,                // 词云图提示框边框宽度
     borderColor: '#000000',        // 词云图提示框边框颜色
     textColor: '#000000',          // 词云图提示框内容文字颜色
     textFont: 14                   // 词云图提示框内容文字字体大小
   },
   series: [                        // 词云图系列
     {
       name: 'xxxx',                // 词云图系列名称
       width: 0,                    // 词云图系列宽度，width 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比
       height: 0,                   // 词云图系列高度，height 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比
       left: 10,                    // 词云图系列离容器左侧的距离，left 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'left', 'center', 'right'
       top: 30,                     // 词云图系列离容器上侧的距离，top 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'top', 'center', 'bottom'
       bottom: 60,                  // 词云图系列离容器下侧的距离，bottom 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'top', 'center', 'bottom'
       right: 10,                   // 词云图系列离容器右侧的距离，right 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比，也可以是 'left', 'center', 'right'
       sizeRange: [6, 48],          // 词云图系列文字大小范围
       rotationStep: 45,            // 词云图系列旋转步长
       rotationRange: [0, 90],      // 词云图系列文字旋转范围
       shape: WordCloudType.CIRCLE, // 词云图系列形状
       showShapeMask: false,        // 词云图系列是否显示背景
       backgroundColor: '#fff2f6f6',// 词云图系列背景颜色
       data: [                      // 词云图系列数据集合
         {
           name: 'test1',           // 词云图系列文字名称
           value: 100,              // 词云图系列数据值
           color: '#5470c6'         // 词云图系列文字颜色
         }
       ]
     }
   ]
...
```
// 为组件设置配置构建类，如果需要在页面初始化就显示图表，则需要在aboutToAppear方法中完成图表数据构建,如果在之后通过事件触发
// 可通过this.model.setOption();来触发数据更新
// 可通过this.model.invalidate();来触发绘制更新
WordCloudChart({ model: this.model })
.width('100%')
.height('70%')
```
### 仪表盘
...
```
import { GaugeChart, GaugeConfig, GaugeChartModel } from '@ohos/mpchart';
  private gaugeConfig: GaugeConfig = {
    //基础配置
    startAngle: 135,           // 起始角度（度），默认135度（左下角）
    endAngle: 45,              // 结束角度（度），默认45度（右下角）
    min: 0,                    // 最小值
    max: 100,                  // 最大值
    //进度条配置
    progress: {
      show: true,              // 是否显示进度条
      roundCap: true,          // 是否使用圆角线帽
      lineStyle: {
        width: 12,             // 进度条宽度
        color: '#5070dd',      // 进度条颜色
      },
    ...
    axisLabel: {
      show: true,              // 是否显示刻度标签
      distance: 12,            // 标签与轴线的距离
      color: '#666666',        // 标签颜色
      fontSize: 14,            // 标签字体大小
      fontWeight: FontWeight.Normal, // 标签字体粗细
      formatter: '{value}',    // 标签格式化器，{value}会被替换为实际值
    },
    //指针配置
    pointer: {
      show: true,              // 是否显示指针
      length: '60%',           // 指针长度，可以是百分比字符串或具体数值
      lineStyle: {
        width: 6,              // 指针宽度
        color: '#5070dd',      // 指针颜色
      },
      image: '',               // 指针图片路径（可选）
      imageWidth: 0,           // 指针图片宽度（可选）
      imageHeight: 0,          // 指针图片高度（可选）
      imageOffsetX: 0,         // 指针图片水平偏移（可选）
      imageOffsetY: 0,         // 指针图片垂直偏移（可选）
    },
    //锚点配置
    anchor: {
      show: true,              // 是否显示锚点（指针中心圆点）
      size: 25,                // 锚点大小
      itemStyle: {
        color: '#ff42ab68',    // 锚点颜色
        borderWidth: 10,       // 锚点边框宽度
        borderColor: '#ffffff', // 锚点边框颜色
      }
    },
    //标题配置
    title: {
      show: false,             // 是否显示标题
      offsetCenter: [0, '10%'], // 标题偏移位置 [水平偏移, 垂直偏移]
      fontSize: 16,            // 标题字体大小
      color: '#333333',        // 标题颜色
    },
    //详情配置
    detail: {
      show: true,              // 是否显示详情（数值显示）
      valueAnimation: true,    // 是否开启数值动画
      fontSize: 40,            // 详情字体大小
      fontWeight: FontWeight.Normal, // 详情字体粗细
      color: '#000000',        // 详情颜色
      offsetCenter: [0, '20%'], // 详情偏移位置 [水平偏移, 垂直偏移]
      formatter: '{value}',    // 详情格式化器，{value}会被替换为实际值
    },
    //数据配置
    data: [
      {
        value: 30,             // 当前数值
        name: 'SCORE'          // 数据名称（会显示在标题中）
      }
    ]
  };
  private model: GaugeChartModel = new GaugeChartModel(this.gaugeConfig);
  // {type}-类型, {value}-值
  // 设置朗读模式图表内容朗读
  this.model.setReadChartText('type: {type}, value: {value}');
```

2. 添加数据到仪表盘组件

```
GaugeChart({ model: this.model })
.height(300)
.width('100%')
...
| -------------- | ------ | ---- | ------------------------------------------------------------ |
| durationMillis | number | 是   | 动画播放的时长，单位毫秒。                                   |
| easing         | string | 是   | [动画差值曲线](https://gitcode.com/openharmony/docs/blob/master/zh-cn/application-dev/reference/apis-arkui/js-apis-animator.md#animatoroptions)。 |
...
| durationMillisX | number | 是   | X轴条目动画播放的时长，单位毫秒。                            |
| durationMillisY | number | 是   | Y轴条目动画播放的时长，单位毫秒。                            |
| easingX         | string | 是   | X轴[动画差值曲线](https://gitcode.com/openharmony/docs/blob/master/zh-cn/application-dev/reference/apis-arkui/js-apis-animator.md#animatoroptions)。 |
...
| durationMillisY | number | 是   | Y轴条目动画播放的时长，单位毫秒。                            |
| easingX         | string | 是   | X轴[动画差值曲线](https://gitcode.com/openharmony/docs/blob/master/zh-cn/application-dev/reference/apis-arkui/js-apis-animator.md#animatoroptions)。 |
| easingY         | string | 是   | Y轴[动画差值曲线](https://gitcode.com/openharmony/docs/blob/master/zh-cn/application-dev/reference/apis-arkui/js-apis-animator.md#animatoroptions)。 |
...
public setOption(funnelOption: FunnelOption): void;
...
| 参数名  | 类型    | 必填 | 说明                      |
| ------- | ------- |----|-------------------------|
| funnelOption | FunnelOption | 是  | 漏斗图配置项。如果传空对象，则使用默认配置项。 |
...
**十二、WordCloudChartModel**

**1.setOption**

setOption(wordCloudOption: WordCloudOption): void;
...
| 参数名  | 类型    | 必填 | 说明                      |
| ------- | ------- |----|-------------------------|
| wordCloudOption | WordCloudOption | 是  | 词云图配置项。如果传空对象，则使用默认配置项。 |
...
setData(roseData: RoseData): void;
...
| 参数名  | 类型    | 必填 | 说明                      |
| ------- | ------- |----|-------------------------|
| roseData | RoseData | 是  | 玫瑰图配置项。如果传空对象，则使用默认配置项。 |
...
| setOption                     | 设置仪表盘配置选项   |
| invalidate                    | 重新绘制仪表盘     |
| setCurrentValue               | 设置仪表盘的数据值   |
| setDetailValueAnimation       | 设置详情值是否启用动画 |
...
提供了两种性能优化选项，用于在处理大数据量时提升图表渲染性能和加载速度。这两个优化功能数据采样默认是**关闭**的，优化加载时间默认是**开启**，开发者根据实际场景手动启用/关闭。

**示例代码：**

完整的性能优化示例可参考以下文件：
- `entry/src/main/ets/customView/PerformanceOptimizationComponent.ets` - 性能优化控制公共组件
- `entry/src/main/ets/pages/lineCharts/MultiplePage.ets` - 线形图性能优化示例
- `entry/src/main/ets/pages/barCharts/BarChartPage2.ets` - 柱状图性能优化示例
- `library/src/main/ets/components/utils/DataSamplingUtils.ets` - 数据采样工具类
...
```typescript
// 创建图表配置类（以 LineChartModel 为例）
this.model = new LineChartModel();

// 启用数据采样优化
this.model.setDataSamplingEnabled(true);
...
```typescript
// 创建图表配置类（以 BarChartModel 为例）
this.model = new BarChartModel();

// 启用优化加载时间
this.model.setLoadTimeOptimizationEnabled(true);
...
// 同时启用两种优化
this.model.setDataSamplingEnabled(true);           // 启用数据采样优化
this.model.setLoadTimeOptimizationEnabled(true);   // 启用优化加载时间
...
## 贡献代码
使用过程中发现任何问题都可以提 [Issue](https://gitcode.com/openharmony-tpc/ohos_mpchart/issues) 给我们，当然，我们也非常欢迎你给我们发 [PR](https://gitcode.com/openharmony-tpc/ohos_mpchart/pulls) 。

## 开源协议
本项目基于 [Apache License 2.0](https://gitcode.com/openharmony-tpc/ohos_mpchart/blob/master/LICENSE) ，请自由地享受和参与开源。